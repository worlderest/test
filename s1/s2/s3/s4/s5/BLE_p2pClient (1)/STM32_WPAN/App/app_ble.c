...
typedef struct
{
  ...
  // uint16_t esp_tx_handle;      // 对应 FFA2，用于接收 ESP32 发来的 Notify 数据
  // uint16_t esp_rx_handle;      // 对应 FFA1，用于 STM32 下发 0x01/0x00 指令
  // uint16_t esp_tx_cccd_handle; // 对应 FFA2 的描述符配置句柄，用于开启 Notify
  
  // uint8_t  discovery_complete; // 标志位：标识服务发现是否彻底完成
  /* USER CODE END PTD_1 */
...
if ((adv_type == HCI_ADV_EVT_TYPE_ADV_IND) || (adv_type == HCI_ADV_EVT_TYPE_SCAN_RSP))
  {
    ...
    while(i < adv_data_size)
    {
      ...
      //     break;
      //   }/* end of switch*/
// 将你 app_ble.c 中的 switch (ad_type) 部分，替换为以下内容：
      ...
      switch (ad_type)
      {
        ...
        // --- 架构师修改区：改为按名字匹配 (Local Name) ---
        case AD_TYPE_COMPLETE_LOCAL_NAME:
        case AD_TYPE_SHORTENED_LOCAL_NAME:
        {
          // 比较广播包里的名字是不是咱们的 ESP_NODE_1
          // 注意：ad_length 包含了类型字本身(1字节)，所以名字实际长度是 ad_length - 1
          if ((ad_length >= 11) && 
              (strncmp((char *)(p_adv_data + i + 2), "ESP_NODE_1", 10) == 0))
          {
            // 匹配成功！记录下这个 ESP32 的 MAC 地址
            bleAppContext.a_deviceServerBdAddrType = p_adv_report->Advertising_Report[0].Address_Type;
            bleAppContext.a_deviceServerBdAddr[0] = p_adv_report->Advertising_Report[0].Address[0];
            bleAppContext.a_deviceServerBdAddr[1] = p_adv_report->Advertising_Report[0].Address[1];
            bleAppContext.a_deviceServerBdAddr[2] = p_adv_report->Advertising_Report[0].Address[2];
            ...
            bleAppContext.deviceServerFound = 0x01; // 立起 Flag：找到了！

            LOG_INFO_APP("  Target Found: ESP_NODE_1, db addr 0x%02X:%02X:%02X:%02X:%02X:%02X\n",
                            bleAppContext.a_deviceServerBdAddr[5],
                            bleAppContext.a_deviceServerBdAddr[4],
                            bleAppContext.a_deviceServerBdAddr[3],
                            bleAppContext.a_deviceServerBdAddr[2],
            ...
            found_status = 1; // 告诉外层函数，可以停止扫描并连接了
          }
          break;
        }
        // ---------------------------------------------
  ...
  if (bleAppContext.deviceServerFound != 0)
  {
    ...
    result = aci_gap_create_connection(SCAN_INT_MS(500u), SCAN_WIN_MS(500u),
                                      ...
                                      //  CONN_INT_MS(50u), CONN_INT_MS(100u),
                                       CONN_INT_MS(15u), CONN_INT_MS(15u),
                                       0u,
