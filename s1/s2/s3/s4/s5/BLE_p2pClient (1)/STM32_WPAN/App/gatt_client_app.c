...
// --- 架构师新增：来自 PC 的指令缓存 ---
uint8_t pc_cmd_buffer = 0x00;
static uint8_t safe_rx_buffer[16];
...
static void My_IMU_Math_Task(void);
/* USER CODE END PFP */
...
void GATT_CLIENT_APP_Init(void)
{
  ...
  UTIL_SEQ_RegTask(1U << CFG_TASK_PARSE_IMU_ID, UTIL_SEQ_RFU, My_IMU_Math_Task);
...
__USED static void gatt_Notification(GATT_CLIENT_APP_Notification_evt_t *p_Notif)
{
  ...
  switch (p_Notif->Client_Evt_Opcode)
  {
    ...
    case P2P_NOTIFICATION_INFO_RECEIVED_EVT:
    {
      ...
      // }
      LOG_INFO_APP("  >> Reveive P2P Notification Events !\n");
      break;
    }
...
/* USER CODE BEGIN gatt_parse_notification_1 */
  ...
  if (index < CFG_BLE_NUM_CLIENT_CONTEXTS)
  {
    // === 判断：是不是 ESP32 通过 FFA2 疯狂推流发过来的数据？ ===
    if (p_evt->Attribute_Handle == a_ClientContext[index].P2PNotificationValueHdl)
    {
//       LOG_INFO_APP("  >> Captured ESP32 , length: %d Byte\r\n", p_evt->Attribute_Value_Length);
      
      // 1. 提取指针和长度
      uint8_t *payload = &p_evt->Attribute_Value[0];
      uint8_t payload_len = p_evt->Attribute_Value_Length;
      
      // 2. 契约校验：检查帧头 0xAA 和长度 16
      if(payload_len == 16 && payload[0] == 0xAA)
      {
          // 架构师的防御性编程：必须使用 static，保证函数退出后内存依然有效
          static uint8_t safe_tx_buffer[16];
          
//          // 架构师的防踩踏机制：检查上一次 DMA 是否搬运完毕！
//          // 如果串口极慢，而蓝牙极快，下一次 Notify 到来时 DMA 还没发完，强行覆盖会导致错包
//          if (huart1.gState == HAL_UART_STATE_READY) 
//          {
//              // 1. 火速将底层易失性内存拷贝到安全的静态缓冲区
//              memcpy(safe_tx_buffer, payload, 16);
//              
//              // 2. 扣动扳机，让 DMA 硬件去搬运安全区的数据，CPU 拍拍屁股走人
//               HAL_UART_Transmit_DMA(&huart1, safe_tx_buffer, 16);
//              // 验证是我们的 16 字节契约数据包
//             if(payload_len == 16 && payload[0] == 0xAA)
//             {
//                 // 1. 光速将数据拷入安全屋
//				memcpy((uint8_t*)safe_rx_buffer, payload, 16);
//              
//          //     // 2. 按下呼叫铃！通知 Sequencer 大总管：“有新数据了，去跑解析任务！”
//				UTIL_SEQ_SetTask(1U << CFG_TASK_PARSE_IMU_ID, CFG_SEQ_PRIO_0);
//              
//          //     // 3. 极其迅速地退出回调，绝不卡住蓝牙底层！
//             }
//               LOG_INFO_APP("  >> [DMA Success] 16 Byte has been received (SEQ: %d)\r\n", (payload[3] << 8 | payload[2]));
//          }
//            else
//            {
//                // 触发此分支说明：上位机波特率太低，或者系统带宽已经到了物理极限！
//                 LOG_INFO_APP("  >> [DMA Warning] UART missed some data!(SEQ: %d)\r\n", (payload[3] << 8 | payload[2]));
//                // printf("X\n");
//                LOG_INFO_APP("X\n");
//            }
      ...
      // 注释掉原本控制板载 LED 的逻辑，因为我们现在是收传感器数据
      /*
      Notification.Client_Evt_Opcode = P2P_NOTIFICATION_INFO_RECEIVED_EVT;
      Notification.DataTransfered.length = p_evt->Attribute_Value_Length;
      Notification.DataTransfered.p_Payload = &p_evt->Attribute_Value[0];
...
static void P2Pclient_write_char(void)
{
  ...
  // 1. 定义一个静态变量来记住当前的状态 (代替原有的 P2PButtonData)
  static uint8_t cmd_value = 0x00; 

  // 2. 状态机翻转 (0x01 启动推流, 0x00 停止推流)
  if (cmd_value == 0x00)
  {
    cmd_value = 0x01;
  }
  ...
  for(index = 0; index < CFG_BLE_NUM_CLIENT_CONTEXTS; index++)
  {
    if (a_ClientContext[index].state == GATT_CLIENT_APP_CONNECTED)
    {
      // 【重点修改】发送长度改为 1 字节，直接发送 cmd_value 的地址
      ret = aci_gatt_write_without_resp(a_ClientContext[index].connHdl,
                                        a_ClientContext[index].P2PWriteToServerValueHdl,
                                        1, 
                                        &cmd_value);
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
//        LOG_INFO_APP(">> send failed!!!! connHdl=0x%04X, ValueHdl=0x%04X\n",
//                a_ClientContext[index].connHdl,
//                a_ClientContext[index].P2PWriteToServerValueHdl);
		  LOG_INFO_APP(">> Fail to send! Error Code: 0x%02X, connHdl: 0x%04X, Handle: 0x%04X\n", 
                  ret, 
                  a_ClientContext[index].connHdl, 
                  a_ClientContext[index].P2PWriteToServerValueHdl);
      }
      ...
      {
        // 【重点修改】把真实发出去的指令值打印出来！
        LOG_INFO_APP(">> SUCCEED TO SEND: 0x%02X\r\n", cmd_value);
      }
    }
  }
...
void My_IMU_Math_Task(void)
{
    // 1. 直接把暂存柜里的数据，扔给我们的独立模块去解析！
    IMU_Parse_Data(safe_rx_buffer);

	memset(safe_rx_buffer, 0, sizeof(safe_rx_buffer));
    // 2. 解析完了？我们可以拿出来看看成果
    float* my_acc = IMU_Get_Acc();
    float* my_gyro = IMU_Get_Gyro();
    
	gyro_Calibration(my_gyro,my_gyro+1,my_gyro+2);
	gyro_DeadZone_execute(my_gyro,my_gyro+1,my_gyro+2);
	EMA_Filter(my_acc,my_acc+1,my_acc+2);
	
	MahonyAHRSupdateIMU(my_gyro[0],my_gyro[1],my_gyro[2],my_acc[0],my_acc[1],my_acc[2]);
