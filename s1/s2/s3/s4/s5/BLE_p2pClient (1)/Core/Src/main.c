...
int main(void)
{
  ...
  MX_ICACHE_Init();
  /* USER CODE BEGIN 2 */
  HAL_UART_Receive_IT(&huart1, &pc_cmd_buffer, 1);
  /* USER CODE END 2 */
...
/* USER CODE BEGIN 4 */
// UART 接收完成回调函数 (当收到 1 个字节时自动触发)
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART1)
  {
    // 1. 契约校验：只有收到 0x01 (启动) 或 0x00 (停止) 时，才理会
    if (pc_cmd_buffer == 0x01 || pc_cmd_buffer == 0x00)
    {
       // 2. 架构师核心逻辑：绝不在中断里强行调用蓝牙 API！
       // 而是向 ST 的 Sequencer (任务调度器) 投递一个“信件”。
       // 当主循环 (while 1) 跑到 MX_APPE_Process() 时，发现有这封信，
       // 就会在安全的上下文中去执行 P2Pclient_write_char 函数。
       UTIL_SEQ_SetTask(1U << CFG_TASK_P2PC_WRITE_CHAR_ID, CFG_SEQ_PRIO_0);
    }
    
    // 3. 重新开启中断，等待 PC 发送下一次指令
    HAL_UART_Receive_IT(&huart1, &pc_cmd_buffer, 1);
  }
}
/* USER CODE END 4 */
